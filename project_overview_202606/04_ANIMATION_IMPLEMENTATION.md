# JDORFX Animation Commands - Implementation Summary

## Overview

Animation functionality has been added to JDORFX by integrating JavaFX's animation framework. The extension provides a set of scriptable(, hardware-accelerated) animation primitives for both 2D and 3D objects, plus higher-level timeline/keyframe support and composition primitives.
...
Primary documentation and examples were added under `samples/`:
- `samples/ANIMATION_COMMANDS_GUIDE.txt` — Full user guide with usage patterns and examples.
- `samples/ANIMATION_COMMANDS_REFERENCE.txt` — Compact command reference.
- `samples/12_jdorfx_animations.rxj` — Executable examples (fade, rotate, pulse, path, combined animations).
- `samples/jdorfx_commands.txt` — Updated master command list.
...
5. Path transitions.
6. Timeline/keyframe animations for multi-property control.
...
1. Run the example script: `samples/12_jdorfx_animations.rxj`
